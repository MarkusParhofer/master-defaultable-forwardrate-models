...
\clearpage
	...
	\begin{comment}
	Assume now that a product is not dependent on the survival of any party. However, the seller of a forward contract can only borrow money at the continuously compounded rate $r_s(t)$, which is higher than the risk free rate $r(t) < r_s(t)$ for all $t\in \left[0,\tilde{T}\right]$.\\
	Then replicating the product comes at a higher cost than what it would be, if the seller could borrow money at the risk free rate.\\
	Lets consider a simple example: the party $A$ sells a forward on an asset with price process $S_t$, strike price $K$ and maturity $\tilde{T}$. We first consider $A$ to be able to borrow at risk-free rate:
	\begin{center}
		\begin{tabular}{c|l}
			Time & Replication \\
			\hline
			$0$ & Borrow $S_0$ amount of money. Buy 1 x $S_0$. \\
			$\tilde{T}$ & Sell asset for $K$. Pay $S_0*\exp(\int_{0}^{\tilde{T}}r(t)dt)$ in redemption.  \\
		\end{tabular}
	\end{center}
	It is easy to see, that the price for such a product therefore must be $S_0 - K\exp(-\int_{0}^{\tilde{T}}r(t)dt)$ (because the seller lends the price of the contract until maturity).
	If we now consider $A$ to only be able to borrow at $r_s(t)$, but lend at $r(t)$, we need to adjust the replication:
	\begin{center}
		\begin{tabular}{c|l}
			Time & Replication \\
			\hline
			$0$ & Borrow $S_0$ amount of money. Buy 1 x $S_0$. \\
			$\tilde{T}$ & Sell asset for $K$. Pay $S_0*\exp(\int_{0}^{\tilde{T}}r_s(t)dt)$ in redemption.  \\
		\end{tabular}
	\end{center}
	Now the price also needs to be adjusted to: $\left(S_0\exp(\int_{0}^{\tilde{T}}r_s(t)dt) - K\right)\exp(-\int_{0}^{\tilde{T}}r(t)dt)$
\end{comment}
