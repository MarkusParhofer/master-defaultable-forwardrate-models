...
\clearpage
	...
	Throughout section \ref{sec:defaultableLMM} we describe the model developed by Prof. Fries and derive some implications including a general pricing methodology in section \ref{sec:pricing}.
	Section \ref{sec:numerical} gives an overview of the actual numerical specification of the model as well as the implementation details in the programming language Java.
	In section \ref{sec:numericalvaluation} we give an overview of numerical results gained by using the model.
	...
	\end{lemma}
	\begin{proof}
		See \cite{fima2Lecture}, p.\ 51. %TODO: Add line where this is done.
	\end{proof}
	\begin{theorem}
	...
	\end{theorem}
	\begin{proof}
		See \cite{fima2Lecture}, p.\ 52. %TODO: Add line where this is done.
	\end{proof}
	...
	\end{lemma}
	\begin{proof}
		See \cite{fima2Lecture}, p.\ 6. %TODO: Add line where this is done.
	\end{proof}
	...
	\begin{lemma}
		...
		\begin{align*}
			W^1_t = \sum_{i=1}^{d}a_iU^i_t\\
			W^2_t = \sum_{i=1}^{d}b_iU^i_t
		\end{align*}
	...
	\end{lemma}
	\begin{proof}
		This is a direct implication from the previous results together with the results on PCA in \cite{FriesBook}, p.\ 471.
	\end{proof}
	...
	The idea of the model is to discretize a given time horizon $\left[0,\tilde{T}\right]$, where $\tilde{T}\in \mathbb{R}$ is the last time point we analyze, into periods, for each of which different rates hold. The main difference to other models, however, is that each rate for a given period is driven by different stochastic parameters.
	\\
	...
	\begin{definition}\label{def:couponbond}
		...
		A \emph{(fixed) coupon bond} with notional $\mathcal{N} \in \mathbb{R}$ and  coupons $c_i \in \mathbb{R}$ for $i\in \{1, ...,N\}$ on the given tenor is a product that pays $c_i$ at each time point $T_i$ and additionally the notional $\mathcal{N}$ at maturity $T_N$.
	\end{definition}
	\begin{remark}
		A variation of this definition is that $c_i$ are defined as coupon rates and the actual coupon payment is then $c_i\, \mathcal{N}$ at each time step $T_i$. Another popular definition includes the terminal payment of the notional $\mathcal{N}$ in the last coupon $c_N$. \\
		Additionally to this "normal" coupon bond one can also find \emph{amortizing} coupon bonds in the market that distribute the notional $N$ in the coupons over all periods instead of paying it all at once at the maturity time.
	\end{remark}
	\begin{lemma}
	...
	\end{lemma}
	\begin{proof}
		This is a well known result of stochastic calculus. A proof can be found in \cite{kloedenSchemes}, pp.\ 131.
	\end{proof}
	\begin{remark}
	...
	\end{align*} 
	With the assumption that the short period bond is deterministic w.r.t. the previous LIBOR time, we can derive the price of a zero-coupon bond for which observation time and maturity lie in the same LIBOR period:
	\begin{lemma}
	...
	Let us look at a specific product category: American loan options. This is the option to buy or sell a loan at a fixed notional, even if the market conditions have changed. We assume an over the counter (OTC) product, meaning trading the product to other counter parties is not possible. The payoff of such a product if exercised at $t$ would be (in this case we can buy the loan):
	\begin{align*}
		\Psi(t) = \left(\mathcal{N} - \Pi_t\right)^+,
	\end{align*}
	where $\mathcal{N}$ is the notional and $\Pi_t$ denotes the current value at time $t$ of the repayment obligation, typically structured as a coupon bond.\\
	...
	To also analyze the Euler scheme analytically, we define the strong rate of convergence as in \cite{kloedenSchemes}:
	\begin{definition}
	...
	\end{proof}
%	\begin{remark}
%		Note that by \cite{strongConvImpliesWeakConv} strong convergence implies weak convergence, which means that for any two times differentiable function $\phi$ (such as $\phi(x) = exp(x)$) the above equation holds as well.\\
%		Hence we also have a strong convergence of the 
%	\end{remark}
	...
	Hence a loan is nothing else than a fixed coupon bond, where the notional $\mathcal{N}$ represents the debt and the coupons $c_i$ represent the interest.
	...
	\end{align}
	where $T_s$ is the start time of the bond and for ease of notation the terminal coupon $c_N$ includes the terminal payment of the notional $\mathcal{N}$.\\
	...
	We get the notional $\mathcal{N}$ at the start of the loan. Then we pay coupons each tenor until $T_k$. If we cancel the loan, we have to pay the redemption $\tilde{R}$, otherwise we keep paying coupons until the end of the loan.
	\begin{figure}[h]
		\centering
		\makebox[\textwidth][c]{%
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	Note that if $\left(\sum_{j=k+1}^{N}c_jP^{d,*}(T_k;T_j) - \tilde{R}\right) > 0$ at $T_k$, the debtor could get a new loan on the notional $\tilde{R}$ for better conditions than the initial one.\\
	...
	\section{Links}
	\begin{itemize}
		\item Github repository with Maven project implemented for this thesis: \color{blue} \href {https://github.com/MarkusParhofer/master-thesis-default-forward-rate-models}{master thesis github repository}\color{black}
		\item Github repository of finmath library: \color{blue} \href{https://github.com/finmath/finmath-lib}{finmath lib github repository}\color{black}
		\item Documentation for finmath library: \color{blue} \href{https://www.finmath.net/finmath-lib/}{finmath lib}\color{black}
	\end{itemize}
