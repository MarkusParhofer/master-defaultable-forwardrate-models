...
\lstset{language=Java,%
	...
	%emph=[1]{for,end,break},emphstyle=[1]\color{orange}, %some words to emphasise
...
\clearpage
	...
	\begin{remark}
		Note that 
		\begin{align*}
			q(T_i, \omega) = \dfrac{1 + \Delta  T_i L_i(T_i)(\omega)}{1 + \Delta  T_i L^d_i(T_i)(\omega)}.
		\end{align*}
		Furthermore, with the definition of $q(T_i, \omega)$ one can write the total survival probability in terms of the numeraire $B(T_i)$ (corresponding to the Spot measure):
		\begin{align*}
			\mathbb{Q}^B\left( \left. \left\{\tau(\omega) > T_{i+1} \right\}  \right| \mathcal{G}_{T_i} \right) = \dfrac{B(T_i)}{\prod_{k=0}^{i}(1 + \Delta T_k L^d_k(T_k)(\omega))} = \dfrac{B(T_i)}{B^d(T_i)}.
		\end{align*}
		The denominator in this equation can be seen as a "defaultable numeraire".
	\end{remark}
	\begin{definition}
	...
	In the case of a non defaultable buyer and a non defaultable seller, "pricing" the loan is therefore nothing else than setting the coupons such that (we assume the same tenor as ):
	...
	The \texttt{ProcessModel} interface is the finMath equivalent of an SDE. It specifies that any implementation has methods for getting the drift and the factor loadings, which can then be used in other implementations e.g. for simulating the specified model.
	...
	\lstinputlisting[caption={Declaration of \texttt{DefaultableLIBORMarketModel}}, label={lst:javaCode}, firstnumber=12, linerange={12} ]{\codedlmm{DefaultableLIBORMarketModel.java}}
	\begin{figure}[h]
		\centering
