...
\clearpage
	...
	\end{figure}
	As time discretization we chose $\Delta t = 0.2$.\\
	...
	In the first subsection we will inspect the performance, so time the model, test the numerical valuation against approximation formulas for two specific products and we test the positivity of the spread in three different cases.
	The second subsection is dedicated to a qualitative analysis of some specific product valuations, which we found to be interesting. Each product here will be first described theoretically and then we analyze what the implementation yields in different cases.
	In the third part we show various numerical results, that are interesting.
	\color{black}
	...
	%TODO: At the conclusion, increase performance by looking at RV CUDA extension and OpenCL extension.
	...
	\begin{figure}[h!]
		...
		\caption{Relative error of defaultable bond prices calculated by a defaultable LIBOR model by a functional Euler-Maruyama scheme with time step $\Delta t=0.02$ and $25000$ sample paths.}
		\label{fig:bondpriceerrorrel}
	\end{figure}
	\cref{fig:bondpriceerrorrel} shows how this error evolves over time.
	\color{red}Extend!!\color{black}
	...
	\begin{align*}
		L^d_i(0)P^d(0;T_{i+1}) \approx \mathbb{E}^{\mathbb{Q}^B}\left[\frac{L^d_i\left(T_{i}\right)}{B(T_{i+1})}\mathbf{1}_{\{\tau > T_{i+1}\}}\right]
	\end{align*}
	The relative error is shown in \cref{fig:liborerrorrel}.
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\linewidth]{figures/Today/LIBORError_rel}
		\caption{Relative error for the approximation of LIBOR rates using a functional Euler-Maruyama schemewith time step $\Delta t=0.02$ and $25000$ sample paths}
	...
	\end{figure}
	Note that the creditor would have a different stopping criterion $\nu$ if it were their option to exercise. Hence, for valuation we can see two different paths: either the creditor values with their stopping criterion or they value with the debtor's criterion. 
	As visualized in \cref{fig:cancellableloanbycouponrate}.
	\color{red}Elaborate!!!\color{black}. 
	%TODO: Elaborate
	
	\subsection{Impact of the model inputs}
	Let us analyze how the initial spread and the matrix of free parameters affect the model. \Cref{fig:spreadvarcovbyfreeparameters} illustrates how the variance of the spread is impacted by the initial spread. We see that the effects are exponential. This is to be expected as the model is lognormal. We also see that the effect is greater for a larger $\Delta f$.
	\begin{figure}[h!]
		\centering
	...
	\end{figure}
	Let us take a look at the correlation of two jointly simulated defaultable models. Also here we are interested in reviewing how the model input affects the values. Remember that in \cref{sec:pricing} we assumed two models for the joint valuation of products to be independent. \Cref{fig:spreadcorrelationbyinitspread} shows that at least numerically this seems to be possible to achieve. Even when using only two additional Brownian Motion factors, there is almost no correlation to be found between the terminal spreads of the two models. We see that the initial spread indeed has an impact on the correlation structure, although on a very low niveau.
	\begin{figure}[h!]
		\centering
