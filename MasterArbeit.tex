...
\clearpage
	...
	Assume now that a product is not dependent on the survival of any party. However, the seller of a forward contract can only borrow money at the continuously compounded rate $r_s(t)$, which is higher than the risk free rate $r(t) < r_s(t)$ for all $t\in \left[0,\tilde{T}\right]$. Then replicating the product comes at a higher cost than what it would be, if the seller could borrow money at the risk free rate.\\
	...
	\end{proof}
	Note that these conditions are the same as for the unique solution theorem. So if these were not satisfied, we could not guarantee that the SDEs of the LIBOR mode have a solution. Hence we assume that the conditions above are fulfilled. 
