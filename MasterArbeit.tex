...
\clearpage
	...
	\begin{proof} 
		...
		\end{align*}
		where (B) comes from (A) and (C) comes from the tower property and the fact that $q(T_1)$ is a constant. Then we repeat these steps until eq. (D) always keeping in mind $q(T_j)$ is $\mathcal{G}_{T_i}$-measurable for all $j \le i$.
	\end{proof}
	\begin{remark}
