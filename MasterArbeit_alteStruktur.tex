...
\clearpage
	...
	\end{align*}
	We see that a whole term is dropped: $X^D$ which is dependent on the default of $c$. Reasoning with funding costs: it is unnecessary to replicate cash flows that go beyond one owns default. Reasoning with the analogy above: $X^D$ is only relevant in the case where $c$ defaults, hence $c$ can neither benefit nor incur liabilities from $X^D$.\\
	\color{red}Add Content!!!\color{black}\\%TODO: Ausführen & Verbessern
	...
	\begin{remark}\label{rem:nativecouponsforloans}
		Setting 
		\begin{align*}
			c_i = \mathcal{N}\Delta T_{i-1}L_{i-1}(0)
		\end{align*}
	...
	\begin{proof}
		We have 
		\begin{align*}
			\Delta T_{i}L_{i}(0) = \frac{P(0;T_{i})}{P(0;T_{i+1})} - 1
		\end{align*}
		Inserting in \cref{eq:pricingnondefaultableloans} yields (note $P(0;T_0) = 1$):
		\begin{align*}
			\sum_{i=1}^{N}\mathcal{N}\Delta T_{i-1}L_{i-1}(0) P(0;T_i) + \mathcal{N}P(0;T_N) \\
	...
	\begin{align*}
		c_i = \mathcal{N}\Delta T_{i-1}L^d_{i-1}(0)
	\end{align*}
	yields a valid loan.\\
	...
	\begin{align*}
		c_i &= \mathcal{N}\Delta T_{i-1}L^d_{i-1}(0) \quad\text{for } i\in\{1,...,N-1\}, \\ 
		c_N &= \mathcal{N}\left(1 + \Delta T_{i-1}L^d_{i-1}(0)\right).
	\end{align*}
	...
	\begin{comment}
	Until now we have not considered \cref{rem:defaultablepartystillcollects}, which states that a party that is in default still collects its claims, not however pays its liabilities. Let us adjust the cash flows in \cref{eq:loanForward,eq:loanForwardCreditorDefaultable} to account for this. Denoting 
	\begin{align*}
		\Psi(T_s) := \left(\mathcal{N} - \sum_{i=s+1}^{N}c_iP^{d^d,*}(T_s; T_i)\right)
	\end{align*}
	we get
	\begin{align}
		\Psi_1(T_s) &= \Psi\left(T_s\right)^+ - \Psi\left(T_s\right)^-\mathbf{1}_{\{\tau^d > T_s\}}\\
		\Psi_2(T_s) &= \Psi\left(T_s\right)^+\mathbf{1}_{\{\tau^c > T_s\}} - \Psi\left(T_s\right)^-\mathbf{1}_{\{\tau^d > T_s\}}.
	\end{align}
	...
	\begin{align*}
		c_i &= \mathcal{N} \Delta T_{i-1} L^d_{i-1}(0) + \frac{\Pi^\Psi}{k P^{d,*}(0;T_i)} \quad &\text{for } i \in \{1,...,k\}\\
		c_i &= \mathcal{N} \Delta T_{i-1} L^d_{i-1}(0) \quad &\text{for } i \in \{k+1,..., N -1\}\\
		c_i &= \mathcal{N} \left(\Delta T_{i-1} L^d_{i-1}(0) + 1\right) \quad &\text{for } i = N
	\end{align*}
